...
/* on commente pour faire un test
...
/*13/08/14: on a rajoute cette ligne*/
/*
%let i_pib=0;
data _null_;
  set dico&&pays&l ;
  if intitule eq "SWDA,MIO_EUR_CLV2005,B1GM,&&pays&l" then call symputx("i_pib",_n_) ;
  %do i=1 %to &&&&nvar&&pays&l ;
  if _n_ eq &i then
  call symputx(cats("var&&pays&l.._",_n_),nomvar) ;
  if dispo eq 1 then
  call symputx(cats("idispo_1_",_n_),dispo);
run ;*/
...
if _n_ eq &i  and dispo eq 1 then call symput("idispo_1_&i",trim(left(dispo))); 
%end ;
run ;

proc freq  ; tables dispo ; 
run ; 
%do i=1 %to 100 ;
%put &&&&idispo_1_&i;
%end ; 
...
/*
13/08/14: on commente cette partie
car plus besoin
*/
