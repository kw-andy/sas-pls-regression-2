...
%macro lapls;
	...
	%do l=1 %to 1 /*5*/
		...
		data dico&&pays&l;
			...
			if ajoutdate=777 then
				do;
					date_publi=intnx('weekday',intnx('month',&ladate,-1,'E'),-1);
					date_serie=date_publi;
				end;
			else if label_eu_oc in ("euro","ocde") then
				do;
					date_publi=intnx('weekday',intnx('month',&ladate,-1,'E'),-1);
					date_serie=date_publi;
				end;
		...
		data dico&&pays&l;
			...
			if ajoutdate eq 777 then
				dispo=0;
			else if (ajoutdate le 40 and date_serie+40>&ladate) then
				dispo=0;
			else dispo=1;
		run;
...
%mend ;
		...
		data dico&&pays&l;
			...
			if intitule eq "SWDA,MIO_EUR_CLV2005,B1GM,&&pays&l" then
				call symput("i_pib",trim(left(_n_)));

			%do i=1 %to &&&&nvar&&pays&l;
				if _n_ eq &i then
					call symput("var&&pays&l.._&i",trim(left(nomvar)));

				if _n_ eq &i then
					call symput("idispo_1_&i",trim(left(dispo)));
			%end;
		run;
		...
		%do la=0 %to 5 ;/* 0*/
			%do lb=0 %to 5;/* 0*/
				...
				data a;
					if _n_ eq 1 then
						set date (rename=(nouv_date=date_prev));
					set in1.a&&pays&l;
					...
					%do j=0 %to 5;
						%do i=1 %to &&&&nvar&&pays&l;
							...
							varnum_&i._&j=&&&&var&&pays&l.._&i.._t&j;
						%end;
					%end;

					act_pib= &&&&var&&pays&l.._&i_pib.._t0;

					if nouv_date ge date_prev then
						&&&&var&&pays&l.._&i_pib.._t0=.;
				run;
			...
			%end;
			;
			output out=min min=%do j=0 %to 5;
				...
				data a;
					...
					%do j=0 %to 5;
						%do i=1 %to &&&&nvar&&pays&l;
							...
							if varnum_&i._&j eq 0 then
								varnum_&i._&j=0.0001;
						%end;
					%end;
				run;

				proc summary nway;
					id qtr_prev ann_prev;
					class annee qtr;
					var act_pib &&&&var&&pays&l.._&i_pib.._t0 
						%do j=0 %to 5;

					%do i=1 %to &&&&nvar&&pays&l;
						%if &i ne &i_pib or &j ne 0 %then
							%do;
								varnum_&i._&j
							%end;
					%end;
	...
	output out=a mean=;
				...
				proc sort data=a;
					by descending annee descending qtr;
				run;
				...
				data a;
					...
					%if &la eq 0 %then
						%do;
							l&&&&var&&pays&l.._&i_pib.._t0=log(&&&&var&&pays&l.._&i_pib.._t0);
						%end;
				...
				%do f=1 %to 10 ;/* 1 */
					...
					output out=a1_1_res&u predicted=prev_pib;
						...
						data a_res&u;
							...
							%if &la eq 0 %then
								%do;
									prev_pib=exp(prev_pib);
								%end;

							%if &la ge 1 %then
								%do;
									if prev_pib gt 0 then
										prev_pib=exp(log((&la/5)*prev_pib)/(&la/5));
									else delete;
								%end;
						run;
						...
						data a_res&u;
							...
							if (annee lt ann_prev) or (annee eq ann_prev and qtr lt qtr_prev) then
								prev_pib=act_pib;
