...
libname in1 
"C:\travail\etudes\projetpib\donnees\fichiersfinaux\20140821\dico1"  ;

libname in3 
"C:\travail\etudes\projetpib\donnees\fichiersfinaux\20140821\dico3"  ;
...
data ladatecomplete;
ladate=input("10/05/2014",ddmmyy10.);
...
data dico&&pays&l;
  set in3.dico&&pays&l;
  if ajoutdate=777 then date_publi=intnx('weekday',intnx('month',&ladate,-1,'E'),-1);
  else if label_eu_oc in ("euro","ocde") then date_publi=intnx('weekday',intnx('month',&ladate,-1,'E'),-1);
  else 
  do i=1 by 1 until (date_publi<&ladate);
    date_publi=intnx(periode,&ladate,-1*i,'E')+ajoutdate;
  end;
  drop i;
  format date_publi date9.;
run;
