...
data dico&i;
infile "C:\travail\etudes\projetpib\donnees\fichiersfinaux\20140731\fich&i..csv" dlm=';' lrecl=900000 obs=1; 
length intitule $50;
do i=1 to 25000 ; 
  input intitule $ @;
  output;
...
PROC IMPORT OUT= fic&i 
            DATAFILE= "C:\travail\etudes\projetpib\donnees\fichiersfinaux\20140731\fich&i..csv"  
            DBMS=DLM REPLACE;
     DELIMITER='3B'x;
	guessingrows=1000 ; 
     GETNAMES=no;
...
format  dern_date_dispo ddmmyy10. date_publi ddmmyy10. label_eu_oc $4.;
run;
...
proc sql;
     ...
     where nom_fic in (select nom_fic from correspondance);
	 update dico&&pays&l as h1
     set label_eu_oc=(select label_eu_oc from correspondance as h2 where h1.nom_fic=h2.nom_fic)
     where nom_fic in (select nom_fic from correspondance);
quit;
...
data in1.calendeuro; set calendeuro;
data in1.calendocde; set calendocde;
